...
| Sync gap | `shifts` / `purchase_orders` / `stock_adjustments` | Shifts, cash movements, purchase orders and stock corrections are local only, and are the last Phase 1 item. Each needs a backend table and endpoint, so they wait on Neon. A shift is per user per day by design, so it must sync for someone who moves between devices mid-day. Until then the export file is the only way they leave the device. |
