...
function SidebarItem({ icon, label, onClick, danger = false }) {
  return (
    ...
    >
      <span className={`shrink-0 ${danger ? "text-red-500" : "text-gray-500"}`}>
        {icon}
      </span>
      {label}
    </button>
...
function App() {
  ...
  return (
    <div className="min-h-screen bg-gray-900">
        ...
        {[
          ...
          return (
            ...
            >
              ...
              <div
                className={`relative transition-all duration-200 ${active ? "text-primary scale-110" : "text-gray-500"}`}
              >
                {tab.icon}
                {tab.badge && (
              ...
              <span
                className={`text-[11px] transition-all duration-200 ${active ? "font-bold text-primary" : "font-normal text-gray-500"}`}
              >
                {tab.label}
              </span>
            </button>
      ...
      {showEtims && <EtimsModal onClose={() => setShowEtims(false)} />}
    </div>
  );
}
